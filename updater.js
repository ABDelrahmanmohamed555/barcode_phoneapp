...
(function(){
  const CURRENT_VERSION = "4.6.2"; // يجب أن يتطابق مع version.json — يُحدثه generate_update.py تلقائياً
  ...
  async function checkForUpdate(manual=false){
    ...
    for(const url of urls){
      try{
        ...
        // فحص APK native أولاً (حتى لو إصدار الويب نفسه) — لو وجد APK أحدث، أرجعه فوراً (أولوية)
        ...
        console.log(`[OTA] local ${stored} vs remote ${data.version} = ${cmp} @ ${url}`);
        if(cmp > 0){
          if(ignore && ignore === data.version && !manual){
            console.log('[OTA] تم تجاهل هذا الإصدار', data.version);
            continue;
          }
          // احتفظ بأعلى إصدار بين كل المصادر
          if(!bestData || compareVersions(data.version, bestData.version) > 0){
            bestData = data;
            bestCmp = cmp;
            bestUrl = url;
          }
        } else {
          console.log('[OTA] لا يوجد تحديث في', url);
        }
      }catch(e){
        lastErr = e;
    ...
    if(manual){
      if(lastErr) toast('فشل فحص التحديث: ' + lastErr.message);
      else toast('التطبيق محدث ✓ (' + stored + ')');
    }
    console.log('[OTA] لا يوجد تحديث في أي مصدر');
    return null;
  }
  ...
  // اعتراض fetch لخدمة أي ملف OTA حتى لو الملف جديد — مع استثناء version.json و updater/sw للسماح بفحص التحديث
  (function(){
    ...
    window.fetch = async function(input, init){
      try{
        ...
        // لا تعترض أبداً فحص التحديث أو ملفات النظام الحرجة
        if(fname === 'version.json' || fname === 'updater.js' || fname === 'sw.js'){
          return origFetch.apply(this, arguments);
        }
        // لا تعترض طلبات Supabase / GitHub API
        if(clean.includes('supabase.co') || clean.includes('api.github.com') || clean.includes('/rest/v1/') || clean.includes('/realtime/')){
          return origFetch.apply(this, arguments);
        }
        ...
        if(otaVal && compareVersions(ver, CURRENT_VERSION) > 0){
          if(fname.endsWith('.js') || fname.endsWith('.css') || fname.endsWith('.html') || fname.endsWith('.json')){
            ...
            const res = await origFetch(otaVal);
            return res;
          }
        }
      }catch(e){}
